...
public class CrudGridSelectJsonField<T extends ToolboxPersistable> extends CustomField<String> {
	...
	public CrudGridSelectJsonField(final String caption, final String btnInitCaption, final CrudGridComponentBuilder<T> crudGridComponentBuilder) {
		super();
	...
	public CrudGridSelectJsonField(final String caption, final CrudGridComponentBuilder<T> crudGridComponentBuilder) {
		super();
		this.setCaption(caption);
		this.crudGridComponentBuilder = crudGridComponentBuilder;
	}
	...
	private void initDialog() {
		...
		if (StringUtils.isNotBlank(this.value)) {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				valueSet.add(jsonArray.getInt(i));
			}
		} else {
			valueSet = new HashSet<>();
		}

		this.crudGridComponent.setSelectedItemsWithIds(valueSet);
		...
		if (this.isEnabled() && !this.isReadOnly()) {
			...
			btnOk.addClickListener(y -> {
				...
				Set<Integer> selectedItemIds = this.crudGridComponent.getSelectedItemIds();

				JSONArray jsonArrayOutPut = new JSONArray();
				for (Integer selectedItemId : selectedItemIds) {
					jsonArrayOutPut.put(selectedItemId);
				}

				this.value = jsonArrayOutPut.toString();
	...
	@Override
	protected void doSetValue(final String valueToBeSet) {
