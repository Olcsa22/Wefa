...
// nem saját kódunk
// custom (kisebb) memória és disk limitek (768 MB és 0.3*768 ha az össze JVM limit 768 alatt
// más nem lett módosítva
...
public class BrokerService implements Service {
    ...
    public SystemUsage getSystemUsage() { // módosított metódus
        try {
            if (systemUsage == null) {
                ...
                systemUsage.getMemoryUsage().setLimit(1024L * 1024 * 768 * 1); // 768 MB
                systemUsage.getTempUsage().setLimit(1024L * 1024 * 1024 * 5); // 5 GB
                systemUsage.getStoreUsage().setLimit(1024L * 1024 * 1024 * 10); // 10 GB
                systemUsage.getJobSchedulerUsage().setLimit(1024L * 1024 * 1024 * 5); // 5 GB
    ...
    protected void checkMemorySystemUsageLimits() throws Exception { // módosított metódus
        final SystemUsage usage = getSystemUsage();
        long memLimit = usage.getMemoryUsage().getLimit();
        long jvmLimit = Runtime.getRuntime().maxMemory();

        if ((1.0d * memLimit) > (0.3d * jvmLimit)) {
            final String message = "Memory Usage for the Broker (" + memLimit / (1024 * 1024)
                    + "mb) is more than 30% of maximum available for the JVM: " + jvmLimit / (1024 * 1024);

            if (adjustUsageLimits) {
                usage.getMemoryUsage().setPercentOfJvmHeap(30);
                LOG.warn(message + " mb - resetting to 30% of maximum available: " + (usage.getMemoryUsage().getLimit() / (1024 * 1024)) + " mb");
            } else {
                LOG.error(message);
                throw new ConfigurationException(message);
            }
